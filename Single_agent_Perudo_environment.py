...
from Perudo_player import ComputerPlayer
...
class Perudo_environment():
	...
	def Perudo_init(self,single_agent):
		#create the space for self.players with dice
		if single_agent:
			self.players.append(Perudo_player(
						name=0, 
						dice_number=self.starting_dice, 
						game=self
					))
			#add the babysitters
			for i in range(0, self.starting_players-1):
	...
	def step(self,raw_action):
		...
		if self.current_player.name == 0:
			...
			reward, done, failed = [],[],[]
			next_bet = self.current_player.make_bet(self.current_bet)
			bet_string = None
			if next_bet == DUDO:
				bet_string = 'Dudo!'
			else:
